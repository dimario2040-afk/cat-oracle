...
async def handle_voice(u,c):
    ...
    try:
        ...
        share_kb=InlineKeyboardMarkup([[InlineKeyboardButton("📤 Сохранить в Избранное", callback_data="save_card")],[InlineKeyboardButton("📢 Отправить другу", callback_data="share_card")]])
        image_path = None
...
async def share_card(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = u.effective_user.id
    data = _share_data.get(user_id)
    if not data or "chat_id" not in data or "message_id" not in data or "file_id" not in data or "cat" not in data:
        await query.edit_message_text("🌫 Карточка утеряна в тумане... Отправь голосовое заново!")
        return
    _pending_share[user_id] = data
    await c.bot.send_message(chat_id=user_id, text="👤 Отправь @username друга, которому хочешь отправить карточку:")
...
async def handle_share_target(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    friend_caption = f"{u.effective_user.first_name} поделился с тобой тотемом «{data['cat']['title']}» 🐾\n\nhttps://t.me/Catgift_bot"
    try:
        await c.bot.send_photo(chat_id=target, photo=data["file_id"], caption=friend_caption)
